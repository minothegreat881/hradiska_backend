/**
 * Opravy utgard-2018 (telo + perex): deixa „tohoročného" → „2018", malé „vám". Embed zachovaný.
 *   node _fix-utgard.mjs [--commit]
 */
...
const REPL = [
  ['tohoročného Utgardu', 'Utgardu 2018'],                          // #1 deixa
  ['Prinášame Vám jeden zo stretov', 'Prinášame vám jeden zo stretov'], // #2 malé v
];
...
async function main() {
  const r = await fetch(`${BASE}/api/blog-posts?filters[slug][$eq]=utgard-2018&populate[blocks][populate]=*&fields[0]=excerpt&fields[1]=documentId`);
  const d = (await r.json()).data?.[0];
  if (!d) { console.error('nenájdený'); process.exit(1); }
  const newExcerpt = ap(d.excerpt || '');
  const outBlocks = (d.blocks || []).map(cleanBlock);
  console.log('aplikovaných:', [...applied].length, '/', REPL.length, '| embed:', outBlocks.some(b => b.__component === 'content.embed'));
